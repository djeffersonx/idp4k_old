...
class PostgreSqlLockManager(
    ...
    @Transactional
    override fun lock(lock: IdempotenceLock): IdempotenceLock {
        require(lock.isPending()) {
            throw LockInvalidStatusException(lock.key, lock.state)
        }

        return findForUpdateBy(lock.key, lock.group)?.let { persistedLock ->

            require(persistedLock.isPending()) {
                throw LockInvalidStatusException(persistedLock.key, persistedLock.state)
            }
    ...
    override fun release(lock: IdempotenceLock, lockState: LockState) {
        ...
        require(lock.isLocked()) {
            throw LockInvalidStatusException(lock.key, lock.state, "release")
        }

        findForUpdateBy(lock.key, lock.group)?.let { persistedLock ->

            require(persistedLock.isLocked()) {
                throw LockInvalidStatusException(persistedLock.key, persistedLock.state, "release")
            }
